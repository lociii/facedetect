...
import cv, json, urllib2, os
from argparse import ArgumentParser
from werkzeug.serving import run_simple
from werkzeug.wrappers import Request, Response
from werkzeug.exceptions import HTTPException, MethodNotAllowed, BadRequest, NotFound, InternalServerError
...
def detect(img):
	...
	while len(faces) == 0 and angle <= 360:
		...
		faces = cv.HaarDetectObjects(img_small, cascade, cv.CreateMemStorage(0), haar_scale, min_neighbors, haar_flags, min_size)
		if faces:
...
if __name__ == '__main__':
	parser = ArgumentParser(
		description="Face detection web service"
	)
	parser.add_argument(
		"--hostname",
		dest="hostname",
